...
spec = do
  describe "existsInstance" $ do
    ...
    it "should fail when provided with an incomplete instance" $
      existsInstance "pred a (a: Int) { a > a " `shouldThrow` isUserError
  describe "getInstances" $ do
    ...
    it "should fail when provided with an incomplete instance under timeout" $
      getInstancesWith cfg { timeout = Just 1000000 } "pred a (a: Int) { a > a "
